...
try {
  ...
  // --- R227: scope your idea — an audit's permissions, a different result ----

  const scoping = await console_(`/api/projects/${project}/runs/ask`, {
    method: 'POST',
    body: JSON.stringify({ prompt: 'split the tool rules move into cards', profile: 'SCOPE' }),
  });
  check('a scoping session has no branch and its own profile',
    scoping.profile === 'SCOPE' && scoping.kind === 'MANUAL' && scoping.branch === null,
    JSON.stringify([scoping.kind, scoping.profile, scoping.branch]));
  const scoper = await asToken(runnerToken, `/api/runners/${runner.id}/claim/${scoping.id}`);
  await asToken(runnerToken, `/api/projects/${project}/runs/${scoping.id}/transition`, {
    state: 'RUNNING',
  });
  const pieces = [];
  for (const title of ['the first piece', 'the second piece']) {
    pieces.push(await asToken(scoper.runToken,
      `/api/projects/${project}/runs/${scoping.id}/proposals`,
      { kind: 'ROADMAP', title, body: '**Build:** it.', section: 'The tool rules move' }));
  }
  check('a scoping session proposes cards under one section, and creates nothing',
    pieces.every((each) => each.kind === 'ROADMAP' && each.section === 'The tool rules move'
      && each.accepted === false)
      && (await console_(`/api/projects/${project}/runs/${scoping.id}`)).created.length === 0,
  ...
  // (`scope-your-idea.test.mjs` reads that off the spawn line), the same
  ...
  // token could.
  const onTheWay = await asToken(scoper.runToken,
    `/api/projects/${project}/runs/${scoping.id}/proposals`,
  ...
  const scopedOne = await console_(
    `/api/projects/${project}/runs/${scoping.id}/proposals/${pieces[0].id}/accept`,
    { method: 'POST', body: JSON.stringify({}) });
  const scopedTwo = await console_(
    `/api/projects/${project}/runs/${scoping.id}/proposals/${pieces[1].id}/accept`,
    { method: 'POST', body: JSON.stringify({}) });
  const cardOne = await console_(`/api/projects/${project}/roadmap/${scopedOne.entryNumber}`);
  const cardTwo = await console_(`/api/projects/${project}/roadmap/${scopedTwo.entryNumber}`);
  ...
  check('and each says it was raised by a scoping session',
    cardOne.audit?.profile === 'SCOPE' && cardTwo.audit?.profile === 'SCOPE'
      && cardOne.audit?.runId === scoping.id,
    JSON.stringify([cardOne.audit, cardTwo.audit]));
  const filedOnTheWay = await console_(
    `/api/projects/${project}/runs/${scoping.id}/proposals/${onTheWay.id}/accept`,
    { method: 'POST', body: JSON.stringify({}) });
  ...
  const scopedNotice = (await console_('/api/inbox')).finished
    .find((each) => each.runId === scoping.id);
  check('a finished scoping session lands in the inbox with its cards counted',
    scopedNotice?.profile === 'SCOPE' && scopedNotice?.findings === 3,
    JSON.stringify(scopedNotice));
  if (scopedNotice) {
    await console_(`/api/inbox/notices/${scopedNotice.id}/seen`, { method: 'POST' });
  }
