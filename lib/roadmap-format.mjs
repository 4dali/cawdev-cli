...
function renderEntry(entry) {
  ...
  //
  // REVIEW does print it, for the mirror-image reason: the branch is the thing
  // somebody is being asked to read, and it is still there until the pull
  // request lands.
  // DONE too — R74. The reviewer said yes, and the branch is still what they
  // said yes to; it stops being printed only once MERGED says where it went.
  if ((entry.status === 'CODING' || entry.status === 'REVIEW' || entry.status === 'DONE')
    && entry.branch) {
