...
function renderEntry(entry) {
  ...
  if (entry.after?.length) {
    ...
    // written and sorted by number, for Related's reasons.
    const written = entry.after
      .map((item, index) =>
        typeof item === 'number'
          ? { number: item, ref: entry.afterRefs?.[index] ?? `R${item}` }
          : { number: item.number, ref: item.ref ?? `R${item.number}` },
      )
