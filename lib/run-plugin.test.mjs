...
import { agentFile, skillFile, writeRunPlugin, yamlScalar } from './run-plugin.mjs';

async function inATempDirectory(t) {
  const directory = await mkdtemp(join(tmpdir(), 'cawdev-run-plugin-'));
...
test('the plugin is a manifest, the agents and the skills', async (t) => {
  ...
  assert.equal(manifest.name, 'cawdev-run');
