...
import {
  CARD_WRITE, CAWDEV_READS, GIT_READS, SUBAGENT_READS, canChangeThings, canRunThings, driftedFrom,
...
//
// The two tests above — "a PLAN stage has nothing that can change anything" and
// "a read-only stage of a coding run cannot comment on the card" — are the half
// of R112's claim that survives, and they stay green WITHOUT the option. These
// four are the exception, and their job is to hold it to exactly one string.

test('a cardless PLAN stage holds the card write and nothing else that writes', () => {
  const without = toolsForStage('PLAN', CODING);
  const with_ = toolsForStage('PLAN', CODING, undefined, { writesItsOwnCard: true });

  // The difference between the two lists is exactly one name. Asserted as a
  // difference rather than as a membership, because "it has roadmap_create"
  // would still pass if the flag had also let `Write` through.
  assert.deepEqual(with_.filter((tool) => !without.includes(tool)), [CARD_WRITE]);
  assert.deepEqual(without.filter((tool) => !with_.includes(tool)), []);

  // And the only writer in the widened list is that one.
  assert.deepEqual(with_.filter(canChangeThings), [CARD_WRITE]);
});

test('VERIFY and MEMORY do not get it, even when the flag is passed', () => {
  // By the time either runs, the card exists — so writing one is not the first
  // step of anything. The switch arm is split for this.
  for (const stage of ['VERIFY', 'MEMORY']) {
    const tools = toolsForStage(stage, CODING, undefined, { writesItsOwnCard: true });
    assert.ok(!tools.includes(CARD_WRITE), `${stage} was given the card write`);
...
test('a PLAN stage that was NOT given the flag still cannot write a card', () => {
  assert.ok(!toolsForStage('PLAN', CODING).includes(CARD_WRITE));
  assert.ok(!toolsForStage('PLAN', CODING, undefined, {}).includes(CARD_WRITE));
  assert.ok(!toolsForStage('PLAN', CODING, undefined, { delegatesReadOnly: true })
    .includes(CARD_WRITE));
});

test('R113 does not fire on the one call the stage was deliberately given', () => {
  const line = `${CARD_WRITE} {"title":"a new thing"}`;
  assert.equal(driftedFrom('PLAN', line, 'RUN', [], { writesItsOwnCard: true }), null);

  // And without the flag it is a sentence, because then it genuinely should not
  // have been possible.
  assert.match(driftedFrom('PLAN', line, 'RUN', []), /should not have been able to/);

  // The flag widens nothing else: a file write in a cardless plan stage is
  // still the note R113 exists for.
  assert.match(
    driftedFrom('PLAN', 'Write src/main.ts', 'RUN', [], { writesItsOwnCard: true }),
    /the PLAN stage called Write/,
  );
});
