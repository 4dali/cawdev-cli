...
test('a gated stage says a person decides, and an automatic one does not', () => {
  ...
  assert.match(said, /1\. \*\*PLAN\*\* — \*\*a person decides here\*\*/);
  assert.match(said, /2\. \*\*IMPLEMENT\*\*$/m);
  // i138. The gate is the DAEMON's: it raises it when the stage's turn ends and
  // holds the next stage until somebody answers. The wording used to tell the
  // session to raise it itself, with `ask_user`, and to `report` each step —
  // tools a stage is not spawned with. A stage told to use a tool it does not
  // hold asks for it, and on a machine that had said "allow everything" that
  // question is exactly what i138 was filed about.
  assert.doesNotMatch(said, /ask_user/);
  assert.doesNotMatch(said, /Report each one/);
});

test('a staged session is told the daemon reports the steps, and what its last message is for',
    () => {
      // i138's other half. Every profile prompt finishes with "then `report`
      // kind done", and `toolsForStage` strips `report` from every stage because
      // "done" ends the RUN. The steps section is where the two are reconciled,
      // and it has to say three things: that `report` is not this step's, what
      // to do instead, and why the last message matters — it is the plan, the
      // briefing, or what a person reads at a gate.
      const said = harnessPrompt({ lifecycle: [{ stage: 'PLAN', gate: 'ASK' }] });
      assert.match(said, /The daemon reports each step as it begins and ends/);
      assert.match(said, /`report` is not among this step's tools/);
      assert.match(said, /say it as your last message instead/);
      assert.match(said, /The step ends when your turn does/);
    });
...
// --- R145: the other plans that name this plan's files ------------------------

test('the collisions land in the SAME block as the plan', () => {
  const said = harnessPrompt({
    plan: {
      body: 'The plan.',
      staleness: null,
      collisions: 'One other live card plans to touch files this plan also names.\n\n'
        + '- R150 — Batch the lookups (IN DEVELOPMENT) — also names `WorkItemPresenter.java`.',
    },
  });

  // Beside the plan, for the reason the staleness line is: a plan and the
  // reason to look around before following it belong in one place.
  const block = said.slice(said.indexOf('## The plan for this card'));
  assert.match(block, /R150 — Batch the lookups/);
  assert.match(block, /also names `WorkItemPresenter\.java`/);
  assert.match(block, /The plan\./);
});

test('a plan nothing else overlaps says nothing about other cards', () => {
  const said = harnessPrompt({ plan: { body: 'The plan.', collisions: null } });
  assert.doesNotMatch(said, /also names/);
});

test('a stale plan with collisions carries both, staleness first', () => {
  const said = harnessPrompt({
    plan: {
      body: 'The plan.',
      staleness: 'This plan was written against abc1234567 and you are working from def4567890.',
  ...
  const stale = said.indexOf('written against abc1234567');
  const collide = said.indexOf('also names');
  const body = said.indexOf('The plan.');
  assert.ok(stale > -1 && collide > -1);
  // The doubt about the plan itself comes first, then what else is moving,
  // then the plan. And only the first is bold: the second is a heads-up.
  assert.ok(stale < collide && collide < body);
  assert.match(said, /\*\*This plan was written against/);
  assert.doesNotMatch(said, /\*\*One other live card/);
});
...
/**
 * R161. A capability the project REQUIRED gets its own heading rather than a
 * suffix on a line in a list of twelve.
 *
 * The pair of tests below is the compatibility claim: the AS_NEEDED wording is
 * byte-for-byte what it was, and every assertion above this comment passes
 * UNMODIFIED. A project that requires nothing — which is every project until
 * somebody says otherwise — gets exactly the prompt it got before the entry.
 */
test('a required expert and a required skill are named under their own MUST heading', () => {
  const prompt = harnessPrompt({
    experts: [
      { qualified: 'cawdev:architect', description: 'Designs how a change fits.', required: true },
      { qualified: 'cawdev:reviewer', description: 'Reviews the diff.' },
    ],
    skills: [
      { qualified: 'cawdev:dataviz', description: 'Charts.', required: true },
      { qualified: 'cawdev:angular', description: 'Components.' },
    ],
  });
  assert.match(prompt, /\*\*Experts you MUST use\*\*/);
  assert.match(prompt, /\*\*Skills you MUST use\*\*/);

  // First, and in its own block — a required one is not buried under the
  // optional ones it is meant to stand out from.
  assert.ok(prompt.indexOf('Experts you MUST use') < prompt.indexOf('**Experts** —'));
  assert.ok(prompt.indexOf('Skills you MUST use') < prompt.indexOf('**Skills** —'));

  // And each is under the right heading, not merely present somewhere.
  const mustExperts = prompt.slice(prompt.indexOf('Experts you MUST use'),
    prompt.indexOf('**Experts** —'));
  assert.match(mustExperts, /`cawdev:architect`/);
  assert.doesNotMatch(mustExperts, /`cawdev:reviewer`/);
});

test('a project that requires nothing gets byte-for-byte the prompt it got before', () => {
  const experts = [{ qualified: 'cawdev:architect', description: 'Designs how a change fits.' }];
  const skills = [{ qualified: 'cawdev:dataviz', description: 'Charts.' }];

  // `required: false` said out loud and `required` absent entirely must be the
  // same prompt: an old claim carries no mode at all.
  assert.equal(
    harnessPrompt({ experts, skills }),
    harnessPrompt({
      experts: experts.map((each) => ({ ...each, required: false })),
      skills: skills.map((each) => ({ ...each, required: false })),
    }),
  );
  assert.doesNotMatch(harnessPrompt({ experts, skills }), /MUST use/);
});

test('an expert a read-only stage cannot reach is not turned into a MUST by the mode', () => {
  // The rule this entry must never break. `cannotDelegate` is what the runner
  // passes when the narrowing left the stage with nothing, and the mode is read
  // AFTER that narrowing — so there is nothing here to require.
  const prompt = harnessPrompt({
    experts: [],
    cannotDelegate: ['This stage cannot delegate; the experts are reached from IMPLEMENT.'],
  });
