...
test('REVIEW keeps its branch, and keeps its section', () => {
  // R64. The mirror image of MERGED above: a card waiting to be read still has
  // the branch somebody is being asked to read, so the file prints it and
  // `--live` checks it. And REVIEW is not a status-derived section — work
  // waiting to be read is still part of the phase it belongs to.
  const entries = [
    {
      number: 64,
  ...
  const first = renderRoadmap(entries, { preamble: PREAMBLE });
  assert.ok(first.includes('Status: REVIEW'));
